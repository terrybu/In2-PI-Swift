...
class AboutPIViewController: ParentViewController, UIScrollViewDelegate, UIGestureRecognizerDelegate{
    ...
    private func devMode(canvasView: UIView) {
        let rectangle = UIView(frame: CGRect(x: 0, y: 0, width: canvasView.frame.width, height: canvasView.frame.height))
        rectangle.backgroundColor = UIColor.clearColor()
        rectangle.userInteractionEnabled = true
        let tapGesture = UITapGestureRecognizer(target: self, action: "devEgg")
    ...
    @objc
    private func devEgg() {
        UIAlertController.presentAlert(self, alertTitle: "UB3<NIM6102", alertMessage: "LUBU2", confirmTitle: "OK")
    }
